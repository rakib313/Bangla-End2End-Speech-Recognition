...
class AudioGenerator():
    def __init__(self, step=10, window=20, max_freq=8000, mfcc_dim=13,
        ...
        #pint("feat_dim:", self.feat_dim)
        self.mfcc_dim = mfcc_dim
        #pnt("mfcc_dim:", mfcc_dim)
        self.feats_mean = np.zeros((self.feat_dim,))
        #rint("Feats_mean", self.feats_mean)
        self.feats_std = np.ones((self.feat_dim,))
        #rint("feats_srd: ", self.feats_std)
    ...
    def get_batch(self, partition):
        ...
        for i in range(0, self.minibatch_size):
            ##print("Paths : ", audio_paths[cur_index+i])
    ...
    def load_train_data(self, desc_file='data-test-clean.json'):
        self.load_metadata_from_desc_file(desc_file, 'train')
        self.fit_train()
        if self.sort_by_duration:
            self.sort_data_by_duration('train')

    def load_validation_data(self, desc_file='data-train-clean.json'):
...
def vis_train_features(index=0):
    ...
    print("vis_spectrogram_feature: ", vis_spectrogram_feature.shape)
    # obtain mfcc
    audio_gen = AudioGenerator(spectrogram=False)
    audio_gen.load_train_data()
    vis_mfcc_feature = audio_gen.normalize(audio_gen.featurize(vis_audio_path))
    mfc = audio_gen.featurize(vis_audio_path)
    #print("mfc")
    #print(mfc[0])
    #print(vis_mfcc_feature[0])
    #print(vis_mfcc_feature.shape)
   ...
   # print(audio_gen.load_train_data())
